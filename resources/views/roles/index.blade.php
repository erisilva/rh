...
@section('content')
    <div class="container-fluid">
        ...
        <x-btn-group label='MenuPrincipal' class="py-1">
            ...
            @can('role-create')
                <a class="btn btn-primary" href="{{ route('roles.create') }}" role="button"><x-icon icon='file-earmark' />
                    {{ __('New') }}</a>
            @endcan
            ...
            @can('role-export')
                <x-dropdown-menu title='Reports' icon='printer'>
                    ...
                    <li>
                        <a class="dropdown-item"
                            href="{{route('roles.export.pdf', ['description' => request()->input('description'), 'name' => request()->input('name')])}}"><x-icon
        ...
        <form method="GET" action="{{ route('roles.index') }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="description" name="description"
                    value="{{ session()->get('role_description') }}">
            </div>

            <button type="submit" class="btn btn-primary btn-sm"><x-icon icon='search' /> {{ __('Search') }}</button>

            <a href="{{ route('roles.index', ['name' => '', 'description' => '']) }}" class="btn btn-secondary btn-sm"
