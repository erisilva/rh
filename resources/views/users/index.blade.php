...
@section('content')
    <div class="container-fluid">
        ...
        <x-btn-group label='MenuPrincipal' class="py-1">
            ...
            @can('user-create')
                <a class="btn btn-primary" href="{{ route('users.create') }}" role="button"><x-icon icon='file-earmark' />
                    {{ __('New') }}</a>
            @endcan
            ...
            @can('user-export')
                <x-dropdown-menu title='Reports' icon='printer'>
                    ...
                    <li>
                        <a class="dropdown-item"
                            href="{{route('users.export.pdf', ['email' => request()->input('email'), 'name' => request()->input('name')])}}"><x-icon
        ...
        <form method="GET" action="{{ route('users.index') }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="email" name="email"
                    value="{{ session()->get('user_description') }}">
            </div>

            <button type="submit" class="btn btn-primary btn-sm"><x-icon icon='search' /> {{ __('Search') }}</button>

            <a href="{{ route('users.index', ['name' => '', 'email' => '']) }}" class="btn btn-secondary btn-sm"
