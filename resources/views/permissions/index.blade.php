    ...
    <div class="container-fluid">
        ...
        <x-btn-group label='MenuPrincipal' class="py-1">
            ...
            @can('permission-create')
                <a class="btn btn-primary" href="{{ route('permissions.create') }}" role="button"><x-icon icon='file-earmark' />
                    {{ __('New') }}</a>
            @endcan
                ...
                <x-dropdown-menu title='Reports' icon='printer'>
                    ...
                    <li>
                        <a class="dropdown-item"
                            href="{{route('permissions.export.csv', ['description' => request()->input('description'), 'name' => request()->input('name')])}}"><x-icon
                                icon='file-earmark-spreadsheet-fill' /> {{ __('Export') . ' CSV' }}</a>
                    </li>

                    <li>
                        <a class="dropdown-item"
                            href="{{route('permissions.export.pdf', ['description' => request()->input('description'), 'name' => request()->input('name')])}}"><x-icon
        ...
        <form method="GET" action="{{ route('permissions.index') }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="name" name="name"
                    value="{{ session()->get('permission_name') }}">
            </div>

            <div class="mb-3">
                <label for="description" class="form-label">{{ __('Description') }}</label>
                <input type="text" class="form-control" id="description" name="description"
                    value="{{ session()->get('permission_description') }}">
            </div>

            <button type="submit" class="btn btn-primary btn-sm"><x-icon icon='search' /> {{ __('Search') }}</button>

            <a href="{{ route('permissions.index', ['name' => '', 'description' => '']) }}" class="btn btn-secondary btn-sm"
